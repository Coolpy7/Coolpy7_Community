...
import (
	"context"
	"errors"
	"github.com/Coolpy7/Coolpy7_Community/lmux"
	...
	"github.com/Coolpy7/Coolpy7_Community/taskpool"
	"log"
	"net"
	"net/http"
	"runtime"
...
const (
	...
	// Else the new connection will be dispatch to ChanListener B and served by poller.
	IOModMixed = 2
...
type Config struct {
	...
	// if the connection's Send-Q is full and the data cached by is
	// more than MaxWriteBufferSize, the connection would be closed.
...
type Engine struct {
	...
	*Config
	...
	// tlsBuffers [][]byte
	// getTLSBuffer func(c *pollio.Conn) []byte
...
// DataHandler .
func (e *Engine) DataHandler(c *pollio.Conn, data []byte) {
	defer func() {
		if err := recover(); err != nil {
			const size = 64 << 10
			buf := make([]byte, size)
			buf = buf[:runtime.Stack(buf, false)]
			log.Printf("execute parser failed: %v\n%v\n", err, *(*string)(unsafe.Pointer(&buf)))
		}
	}()
	parser := c.Session().(*Parser)
	if parser == nil {
		log.Println("nil parser")
		return
	}
...
// TLSDataHandler .
func (e *Engine) TLSDataHandler(c *pollio.Conn, data []byte) {
	defer func() {
		if err := recover(); err != nil {
			const size = 64 << 10
			buf := make([]byte, size)
...
func (engine *Engine) AddTransferredConn(nbc *pollio.Conn) error {
...
func NewEngine(conf Config) *Engine {
	...
	if conf.ReadBufferSize <= 0 {
		conf.ReadBufferSize = pollio.DefaultReadBufferSize
	}
		...
		go func() { // avoid deadlock
			defer func() {
				if err := recover(); err != nil {
					...
					buf := make([]byte, size)
					buf = buf[:runtime.Stack(buf, false)]
					log.Printf("ClientExecutor call failed: %v\n%v\n", err, *(*string)(unsafe.Pointer(&buf)))
				}
			}()
			f()
		}()
	...
	g.OnData(func(c *pollio.Conn, data []byte) {
		c.DataHandler(c, data)
	})

	engine.isOneshot = conf.EpollMod == pollio.EPOLLET && conf.EPOLLONESHOT == pollio.EPOLLONESHOT && runtime.GOOS == "linux"
	if engine.isOneshot {
...
func SyncExecutor(f func()) bool {
	defer func() {
		if err := recover(); err != nil {
			...
			buf := make([]byte, size)
			buf = buf[:runtime.Stack(buf, false)]
			log.Printf("ProtocolExecutor call failed: %v\n%v\n", err, *(*string)(unsafe.Pointer(&buf)))
		}
	}()
	f()
	return true
