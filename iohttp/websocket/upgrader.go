...
func (u *Upgrader) Upgrade(w http.ResponseWriter, r *http.Request, responseHeader http.Header, args ...interface{}) (*Conn, error) {
	...
	getParser := func() {
		var nbResonse *iohttp.Response
		nbResonse, ok = w.(*iohttp.Response)
		if ok {
	...
	switch vt := conn.(type) {
	case *pollio.Conn:
		parser, ok = vt.Session().(*iohttp.Parser)
		if !ok {
			return nil, u.returnError(w, r, http.StatusInternalServerError, err)
		}
	...
	case *tls.Conn:
		...
		nbc, ok = vt.Conn().(*pollio.Conn)
		if !ok {
			// 2.1 The conn may be from std's http.Server.Serve(llib's tls.Listener),
			if transferConn {
				// 2.1.1 Transfer the conn to poller.
				nbc, err = pollio.EPConn(vt.Conn())
				if err != nil {
					return nil, u.returnError(w, r, http.StatusInternalServerError, err)
				}
				vt.ResetRawInput()
				parser = &iohttp.Parser{Execute: nbc.Execute}
				if engine.EpollMod == pollio.EPOLLET && engine.EPOLLONESHOT == pollio.EPOLLONESHOT {
					parser.Execute = iohttp.SyncExecutor
				}
				wsc = NewConn(u, vt, subprotocol, compress, false)
				nbc.SetSession(wsc)
				nbc.OnData(func(c *pollio.Conn, data []byte) {
					defer func() {
					...
					for {
						...
						if nread > 0 {
							...
							if err != nil {
								log.Printf("websocket Conn Read failed: %v", errRead)
			...
			parser, ok = nbc.Session().(*iohttp.Parser)
			if !ok {
				return nil, u.returnError(w, r, http.StatusInternalServerError, err)
			}
			wsc = NewConn(u, conn, subprotocol, compress, false)
...
func (u *Upgrader) commResponse(conn net.Conn, responseHeader http.Header, challengeKey, subprotocol string, compress bool) error {
	...
	if u.KeepaliveTime <= 0 {
		conn.SetReadDeadline(time.Now().Add(iohttp.DefaultKeepaliveTime))
	} else {
