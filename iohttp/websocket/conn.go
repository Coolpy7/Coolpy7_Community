...
func (c *Conn) handleDataFrame(p *iohttp.Parser, opcode MessageType, fin bool, data []byte) {
...
func (c *Conn) handleMessage(p *iohttp.Parser, opcode MessageType, body []byte) {
	if c.isBlockingMod {
...
func (c *Conn) handleProtocolMessage(p *iohttp.Parser, opcode MessageType, body []byte) {
	if c.isBlockingMod {
		c.handleWsMessage(opcode, body)
...
func (c *Conn) handleWsMessage(opcode MessageType, data []byte) {
	...
	case FragmentMessage:
		log.Println("invalid fragment message")
		c.Conn.Close()
	default:
		c.Conn.Close()
	}
